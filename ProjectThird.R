...
query_11 <- full_join(items, orders, by = 'order_id') %>%
  group_by(order_date) %>%
  mutate(product_count = sum(quantity)) %>%
  mutate(month = month(order_date), year = year(order_date))

ggplot(query_11, aes(x = month, color = factor(store_id))) +
  geom_bar() +
...
query_12 <- full_join(items, orders, by = 'order_id')
...
## Total Revenue ##
query_13 <- full_join(items, orders, by = 'order_id')
...
query_14 <- full_join(items, orders, by = 'order_id')
